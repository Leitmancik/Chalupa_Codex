...
def render():
    st.html('<div class="hero"><div class="eyebrow">MALÝ ÚNIK Z KAŽDODENNOSTI</div>'
            '<h1>Váš čas. Vaše <em>pohoda.</em></h1>'
            '<p>Vyberte si volný termín na chalupě. Cenu uvidíte předem, '
            'rezervaci vám následně potvrdí majitel.</p></div>')
    ui.show_flash()
...
def booking(reservations, prices):
    ...
    start = fields[0].date_input('Příjezd', key='arrival', min_value=today(),
                              ...
                              format='DD.MM.YYYY', value=None)
    valid = start and end and end > start and (end - start).days <= 365
    ...
    if valid:
        ...
        st.html('<div class="booking-empty">Váš odpočinek začíná termínem.<br>'
                'Vyberte příjezd a odjezd v kalendáři<br>nebo v polích výše.</div>')
    st.divider()
    with st.form('guest_form', clear_on_submit=False):
        ...
        submitted = st.form_submit_button('Požádat o rezervaci →', type='primary',
                                         width='stretch', disabled=not valid)
        st.caption('Zatím nic neplatíte. Rezervace čeká na potvrzení majitelem. '
                   'Automatické e-maily zatím neodesíláme.')
    if submitted:
