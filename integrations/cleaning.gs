 ...
 * Nastavení v Script Properties: SHEET_ID, APP_URL, API_SECRET, TOKEN_SECRET, RESEND_API_KEY.
...
function setup() {
  return locked_(() => {
    const p = properties_();
    for (const key of ['SHEET_ID', 'APP_URL', 'API_SECRET', 'TOKEN_SECRET', 'RESEND_API_KEY']) if (!p[key]) throw new Error('Chybí nastavení ' + key);
...
function email_(offer, person) {
  const base = properties_().APP_URL.replace(/\/$/, '') + '/?cleaning_token=';
  ...
  return {from:'Chalupa Vernířovice <uklidvernirovice@libertionova.com>', to:[person[1]], reply_to:'adam.leitmancik@libertionova.com',
